...
## Overview
X25519 is **Diffie-Hellman done on the elliptic curve Curve25519** (an ECDH scheme). It has the same goal as classic DH (menu 8) — agree a shared secret over an open channel — but uses curve *scalar multiplication* instead of modular exponentiation. It is the default key exchange in TLS 1.3, Signal, WireGuard, and modern SSH. CryptoLens walks the full flow: clamped key generation, public keys, the shared secret, HKDF, and an AES-GCM demo.

> **Key agreement, not encryption.** X25519's output is a shared secret, which a KDF turns into a symmetric key for an AEAD cipher. On its own it provides **no authentication** — it must be paired with certificates/signatures to stop man-in-the-middle attacks.

## Why It Replaced Classic DH
- **Smaller:** 32-byte keys vs 256+ bytes for 2048-bit DH, at equal-or-better security.
- **Faster:** curve scalar multiplication beats big modular exponentiation.
- **Safer by design:** one fixed, vetted curve — no primes/parameters to choose wrong — and constant-time by construction, so timing attacks don't leak the key.

## Clamping (unique to Curve25519)
A private key is 32 random bytes, then a few fixed bits are forced ("clamped"):
```
byte[0]  &= 248   // clear low 3 bits → scalar is a multiple of the cofactor 8
byte[31] &= 127   // clear top bit    → scalar < 2^255
byte[31] |= 64    // set bit 254      → fixed high bit ⇒ constant-time ladder
```
This removes whole classes of implementation bugs (small-subgroup issues, variable-length scalars that leak timing) automatically.

## The Math
```
Public key:     A = a·G          (G = fixed base point, u = 9)
                B = b·G
Shared secret:  Alice: a·B = a·(b·G)
                Bob:   b·A = b·(a·G)
                both equal (a·b)·G
```
Recovering the scalar `a` from the point `A` is the **elliptic-curve discrete logarithm problem** — infeasible. An eavesdropper sees `A` and `B` but cannot compute `(a·b)·G`.

## Features
- Clamped private-key generation with the clamping explained
- Public keys via scalar multiplication of the base point
- Shared-secret derivation (ECDH) with match verification
- HKDF-based key derivation feeding AES-GCM
- Honest performance comparison vs a 2048-bit modular exponentiation
- Security guidance (authentication, PFS, constant-time) and the TLS 1.3 flow
